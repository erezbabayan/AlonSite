...
**Pages** (all server-rendered HTML, no client routing): [index.html](index.html) at the root, plus [pages/gallery.html](pages/gallery.html), [pages/letters.html](pages/letters.html), [pages/stories.html](pages/stories.html) (curated quote cards about Alon, sourced from the letters data), [pages/legacy.html](pages/legacy.html) (memorial/legacy photo gallery with press section), [pages/sections.html](pages/sections.html) (letters/videos/audio hub), and [pages/disaster.html](pages/disaster.html) (the helicopter disaster memorial page). Each page is a standalone HTML file with its own `<head>`/SEO block and pulls in `shared.css` plus a page-specific stylesheet (e.g. `css/gallery.css`).
